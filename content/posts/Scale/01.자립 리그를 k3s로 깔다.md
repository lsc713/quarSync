...
graph TB
  ...
  k6["k6 부하생성기<br/>.20"]:::ext
  tr["Traefik · klipper-lb<br/>노드:80"]:::infra
  psvc["payment Service<br/>ClusterIP :8080"]:::svc

  subgraph server["k3s-server · .10"]
    cp_["control-plane · taint<br/>(워크로드 없음)"]:::cp
  end

  subgraph pay_pool["payment 전용 노드풀 · c7g.xlarge x3"]
    pa1["payment 파드<br/>pay-1 · .11"]:::svc
    pa2["payment 파드<br/>pay-2 · .12"]:::svc
    pa3["payment 파드<br/>pay-3 · .13"]:::svc
  end

  subgraph work_pool["workload 노드풀 · m7g.xlarge x3"]
    kb["Kafka 3-broker<br/>노드당 1"]:::infra
    etc["risk x2 · merchant-limit x2<br/>order x2 · redis"]:::infra
  end

  subgraph db_ext["외부 EC2 · 클러스터 밖 (병목 통제변수)"]
    mp[("mysql-payment<br/>.30")]:::db
    mr[("mysql-risk<br/>.31")]:::db
    cd[("cold-db · .32<br/>merchant + order")]:::db
  end

  k6 -->|"POST /cancel"| tr
  tr -->|"Ingress"| psvc
  psvc -.->|"kube-proxy L4"| pa1
  psvc -.-> pa2
  psvc -.-> pa3
  pa1 -->|"사설 IP 직결"| mp
  etc --> mr
