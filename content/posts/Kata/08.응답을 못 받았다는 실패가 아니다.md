---
title: 08. 응답을 못 받았다는 실패가 아니다 — UNKNOWN이라는 세 번째 상태
created: 2026-07-06
tags:
  - kata
  - external-call
...
다만 지금 이 "PG가 키를 기억하고 재생한다"는 아직 **가정**이다 — Fake 어댑터에게 그렇게 답하라고 시켰을 뿐, 진짜 PG가 같은 멱등키를 존중하는지는 확인 전이다. 이 가정은 12편에서 실물 토스에 같은 `cancelId`로 두 번 취소를 쏴 **둘 다 COMPLETED·같은 승인번호**가 오는 걸로 실증한다. 설계는 실물에 붙기 전까진 '아마도'다("그럴 것이다가 아니라 이렇게 나왔다").
